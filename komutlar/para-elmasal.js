...
exports.run = async (client, message, args) => {
  ...
  if (!miktar) return message.reply('<a:yok:840374010637451333> Alacağın miktarı yazman gerek!');


  if (miktar.match(x)) return message.reply('<a:yok:840374010637451333> Tek zeki sensin zaten!');
if (isNaN(args[0])) return message.channel.send("<a:yok:840374010637451333> Lütfen bir sayı gir.")
...
//  let sigara = 1
  ...
  let düşE = 49
    ...
    if (altın < elmaspara*miktar) {
      message.channel.send(`<a:yok:840374010637451333> ${miktar} elmas alabilmek için yeterli altın görünmüyor. \n Gerekli altın **${miktar*elmaspara} **`)
     ...
     .setColor('RANDOM')
     .setDescription(`<a:var:840374348745539614> Başarıyla marketten **${miktar}** elmas aldınız\n Hesabınızdan eksilen altın miktarı: **${miktar*elmaspara}**`)
     .setTimestamp()
