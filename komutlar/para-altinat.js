...
exports.run = async (client, message, args) => {
  ...
  if (user.id === message.author.id) return message.reply(`<:hayir:839223750457163867> Kendine altınmı yollayacaksın ciddimisin?`);
...
if (isNaN(args[1])) return message.channel.send(" Lütfen bir sayı gir.")
  ...
  if (altın < mesaj) {
    ...
    message.channel.send(`<a:var:840374348745539614> Yeterince altının bulunmuyor, sende olan altın:  ${bulunanP === null  ? "0" : `${bulunanP}`}`)
} else if  (altın > mesaj) {

  const embed = new Discord.MessageEmbed()
  .setColor('RANDOM')
  .setDescription(`<a:var:840374348745539614> ${user} Adlı kullanıcıya altın yollandı, yollanılan miktar: ${mesaj}`)
